...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim, nhead, num_layers, dim_feedforward=2048, dropout=0.1):
        super(TransformerEncoder, self).__init__()
        encoder_layer = nn.TransformerEncoderLayer(d_model=input_dim, nhead=nhead, dim_feedforward=dim_feedforward, dropout=dropout)
...
class ResBlock(nn.Module):
    def __init__(self, channels, blocks=3, resscale=0.1, kernel_size=0, dilations=[2], gatedconv=False, enhance='de',
                 ppm=True, transformer_params=None):
        super(ResBlock, self).__init__()
        self.convs = nn.ModuleList(
            [self._build_layer(channels, kernel_size, dilations, gatedconv, enhance, transformer_params) for i in range(blocks)]
        )
        self.resscale = resscale
        self.ppm = PyramidPooling(channels, channels, ct_channels=channels) if ppm else None

    def _build_layer(self, channels, kernel_size=0, dilations=[2], gatedconv=False, enhance='de',transformer_params=None):
        conv = GatedConv if gatedconv else ConvNormAct
        layer = nn.Sequential(
        ...
        elif enhance == 'transformer' and transformer_params is not None:
            layer.add_module('transformer', TransformerEncoder(
    ...
    def forward(self, x):
        for conv in self.convs:
            if hasattr(conv, 'transformer'):
                # 调整维度以适应 Transformer
                x = x.permute(2, 3, 0, 1)  # (B, C, H, W) -> (H, W, B, C)
                x = x.flatten(0, 1)  # (H, W, B, C) -> (H*W, B, C)
                x = conv(x)
                x = x.view(x.size(0) // x.size(1), x.size(1), -1).permute(1, 2, 0)  # (H*W, B, C) -> (B, C, H*W)
                x = x.view(x.size(0), x.size(1), int(math.sqrt(x.size(2))), int(math.sqrt(x.size(2))))  # (B, C, H, W)
            else:
                x = conv(x)
            x = x + x * self.resscale
...
class RefinedNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, enhance='de', ppm=True, bilinear=True):
        ...
        # four-th encoder
        self.resblock = ResBlock(d_channels[2], blocks=3, resscale=0.1, kernel_size=3, gatedconv=True, enhance=enhance,
                                 ppm=ppm, dilations=[2, 4],transformer_params={
        'input_dim': 64,
        'nhead': 8,
        'num_layers': 6,
        ...
        # first decoder
        self.up1 = UpBlock(u_channels[0], u_channels[1], u_channels[1], add_channels=d_channels[1], kernels=[5, 5],
                           gatedconv=False, bilinear=bilinear)
        # second decoder
        self.up2 = UpBlock(u_channels[1], u_channels[2], u_channels[2], add_channels=d_channels[0], kernels=[5, 5],
                           gatedconv=False, bilinear=bilinear)
