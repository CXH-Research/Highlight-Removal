...
parfor i_img = 1:length(listing)
    ...
    try
        ...
        sfi = fix(im2double(imread(fullfile(Image_dir,listing(i_img).name))), @Yoon2006);
    end
...
Theta = clip(imfilter(I, H_h_emph, 'symmetric'), 0, 1);

while true
    Upsilon_d = clip(imfilter(I_d, H_h_emph, 'symmetric'), 0, 1);
    Upsilon_s = clip(imfilter(I_s, H_h_emph, 'symmetric'), 0, 1);
